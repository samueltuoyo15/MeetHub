...
const CreateMeet = () => {
  // States for modals, toggle join room and room ID
  ...
  // Reference for modal click detection
  const modalRef = useRef<HTMLDivElement>(null);

  // Toggle 'New Meeting' modal
  ...
  // Open 'Create Meeting' modal and generate room ID
    ...
    <>
      ...
      <section className="bg-gray-900 min-h-screen p-4">
        ...
        <figure className="mt-32 md:mt-44 mx-auto block text-center text-white">
          ...
          />
          <figcaption className="font-bold text-2xl">Get a link you can share</figcaption>
          <figcaption className="text-lg">
            Tap <span className="font-bold">New meeting</span> to get a link you can send to people you want to meet with all over the world!
          </figcaption>
        </figure>
      </section>

      {/* Overlay when modal is open */}
      {isCreateNewMeeting && <div className="fixed inset-0 bg-black opacity-50 z-10" />}

      {/* New meeting modal */}
      {isCreateNewMeeting && (
        <div
          ref={modalRef}
          style={{ transform: isCreateNewMeeting ? 'translateY(0)' : 'translateY(100%)' }} 
      ...
      {createNewMeetingModal && (
        <div 
        className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 bg-gray-800 text-white rounded-3xl p-6 z-30 transition-all duration-700 ease-in-out">
